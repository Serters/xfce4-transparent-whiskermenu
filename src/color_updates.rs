...
pub fn update_whiskar_menu() -> Result<(), Box<dyn Error>> {
    ...
    theme_content = base_menu_re.replace_all(&theme_content, |caps: &regex::Captures| {
        caps[0].replace(&caps[1], &base_new_color)
    }).to_string();

    theme_content = menu_opacity_re.replace_all(&theme_content, |caps: &regex::Captures| {
    ...
    for entry in fs::read_dir(whisker_menu_dir)? {
        ...
        if let Some(file_name) = path.file_name().and_then(|n| n.to_str()) {
            if whisker_menu_pattern.is_match(file_name) {
                ...
                content = menu_opacity_re.replace_all(&content, |_caps: &regex::Captures| {
                    format!("menu-opacity={}", menu_opacity)
                }).to_string();
